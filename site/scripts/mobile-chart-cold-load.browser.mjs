...
test('mobile cold load retains chart statuses and renders scrollable charts after JavaScript arrives', { timeout: 60_000 }, async () => {
  ...
  try {
    ...
    await page.locator('#calendar-chart [role="status"]').waitFor({ state: 'visible', timeout: 10_000 });
    await page.locator('#histogram-chart [role="status"]').waitFor({ state: 'visible', timeout: 10_000 });
...
test('a failed calendar module leaves the histogram and map able to initialize', { timeout: 60_000 }, async () => {
  execFileSync(process.execPath, ['./node_modules/astro/bin/astro.mjs', 'build'], {
    cwd: siteDir,
    stdio: 'inherit',
  });
